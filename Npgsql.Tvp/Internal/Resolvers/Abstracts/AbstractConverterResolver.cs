...
namespace Npgsql.Tvp.Internal.Resolvers.Abstracts
{
    internal abstract class AbstractConverterResolver<TParameter>(PgSerializerOptions options) : PgConverterResolver<TParameter>
    {
        ...
        {
            return GetDefault(options.GetArrayTypeId(new DataTypeName(GetDataTypeName(value) ?? throw new InvalidOperationException("The parameter must provide the data type name"))));
        }
    }
}